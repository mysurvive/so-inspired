...
Hooks.on("init", () => {
  ...
  game.settings.register("so-inspired", "disableAdd", {
    name: "Disable Player Add Inspiration Button",
    hint: "When true, disables the ability for players to add their own inspiration",
    scope: "world",
    config: true,
    type: Boolean,
    default: false,
...
function renderNewInspoSheet(_sheet, html) {
  if (_sheet.actor.type === "character") {
    ...
    if (actorOwner) {
      ...
      const isDisabled =
        game.settings.get("so-inspired", "disableAdd") == true
          ? "disabled"
          : "";
      ...
      if (_sheet.options.classes.includes("dnd5e2")) {
        ...
        }%"><div class="label"><span class="value">${currentInspiration}</span><span class="separator"> / </span><span class="max">${maxInspiration}</span></div><div class="inspo-buttons"><button type="button" class="add-inspiration-btn" ${isDisabled}><i class="fa-solid fa-plus" style="color: #000000;"></i></button><button type="button" class="remove-inspiration-btn"><i class="fa-solid fa-minus" style="color: #000000;"></i></button></div></div></div>`;
